...
def pca_figure(pca_df: pd.DataFrame, explained, color="condition"):
    ...
    fig = px.scatter(
        pca_df.reset_index(names="sample"), x="PC1", y="PC2", color=color,
        text="sample", hover_data=[c for c in ["condition", "batch"] if c in pca_df.columns],
...
def volcano_figure(res: pd.DataFrame, padj_cut: float, lfc_cut: float):
    ...
    fig = px.scatter(
        d, x="log2FoldChange", y="minus_log10_padj", color="status",
        hover_name="Geneid", hover_data=["baseMean", "pvalue", "padj"],
        category_orders={"status": ["NS", "UP", "DOWN"]},
        labels={"log2FoldChange": "log2 fold-change", "minus_log10_padj": "−log10 adjusted p-value"},
    )
...
def heatmap_figure(matrix_gxs: pd.DataFrame, sample_metadata: pd.DataFrame, zscore=True, title="Heatmap"):
    d = matrix_gxs.copy().astype(float)
    if zscore:
        sd = d.std(axis=1).replace(0, np.nan)
        d = d.sub(d.mean(axis=1), axis=0).div(sd, axis=0).fillna(0)
    fig = go.Figure(data=go.Heatmap(z=d.values, x=d.columns, y=d.index, colorbar={"title": "Z" if zscore else "Expression"}))
    fig.update_layout(title=title, height=max(500, min(1200, 18 * len(d) + 250)), xaxis_title="Samples", yaxis_title="Genes")
    return fig

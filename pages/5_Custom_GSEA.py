...
st.set_page_config(page_title="Custom GSEA · BulkRNA Explorer", page_icon="🧬", layout="wide")
st.title("🧬 Custom GSEA")
st.caption("Run GSEA only on pathways you choose, or create your own gene sets.")


@st.cache_data(show_spinner=False, ttl=24 * 3600)
def cached_gene_set_library(friendly_name: str, species: str):
    return load_library_gene_sets(friendly_name, species)


def _organism() -> str:
    label = st.sidebar.selectbox("Organism for gene sets", ["Human", "Mouse"], index=0, key="custom_gsea_organism")
    return label.lower()
...
def _comparison_context():
    ...
    if not history:
        st.info("Run at least one differential-expression comparison in the main BulkRNA Explorer page first.")
        st.stop()
    if annotation is None:
        st.warning("Gene-symbol annotation is unavailable. Custom GSEA needs gene symbols.")
        st.stop()
    ...
    return comparison, bundle, annotated_res, rank, annotation
...
def _plot_results(results: pd.DataFrame, title: str):
    ...
    if "FDR q-val" in plot_df.columns:
        ...
        fig = px.scatter(
            ...
            y="Term",
            size="_sig",
            hover_data=[c for c in ["NOM p-val", "FDR q-val", "Lead_genes", "Matched genes"] if c in plot_df.columns],
            title=title,
        )
    else:
...
species = _organism()
comparison, bundle, annotated_res, rank, annotation = _comparison_context()
...
if mode == "Select specific pathways":
    st.subheader("Select specific pathways from a curated collection")
    st.write("Choose a collection, search for a biological concept, then click only the pathways you want to test.")
    ...
    permutations = c2.selectbox("Permutations", [100, 250, 500, 1000], index=2, key="selected_pathway_permutations")

    try:
        with st.spinner("Loading pathway names…"):
            library_sets, actual_library = cached_gene_set_library(collection, species)
        ...
        if use_all_filtered:
            ...
            st.write(f"**{len(selected_terms)} pathways selected.**")
        else:
        ...
        if selected_sets:
            ...
            st.dataframe(preview, use_container_width=True, hide_index=True, height=min(380, 35 * len(preview) + 80))
            st.download_button(
        ...
        a1, a2 = st.columns(2)
        min_size = a1.number_input("Minimum matched genes per pathway", min_value=3, max_value=100, value=10, step=1)
        max_size = a2.number_input("Maximum matched genes per pathway", min_value=20, max_value=5000, value=1000, step=10)
            ...
            if not selected_sets:
                ...
                try:
                    ...
                    st.session_state["custom_gsea_last"] = {
                        ...
                        "mode": "selected_pathways",
                        "label": label,
                        "results": results,
                        "gene_sets": selected_sets,
                    }
    ...
    st.subheader("Create your own gene set(s)")
    st.write(
        "Paste a gene list and give it a biological-process name, or upload a TXT/CSV/TSV file. "
        "Gene symbols and Ensembl gene IDs are both accepted."
    )

    n1, n2 = st.columns([2, 1])
    custom_name = n1.text_input("Gene-set / process name", value="Senescence", key="custom_gene_set_name").strip() or "Custom_gene_set"
    permutations = n2.selectbox("Permutations", [100, 250, 500, 1000], index=2, key="custom_gene_permutations")
    ...
    uploaded = st.file_uploader(
        ...
        help="One-column files create one gene set. A two-column file with pathway/gene (or process/gene) creates multiple gene sets at once.",
    )
    ...
    if resolved_sets:
        ...
        if (preview["genes_matched"] < 15).any():
            st.warning("At least one custom gene set has fewer than 15 matched genes. GSEA can run, but small gene sets can give unstable results.")
    ...
    if st.button("▶ RUN MY CUSTOM GSEA", type="primary", use_container_width=True):
        if not resolved_sets:
            ...
            if not passing:
                st.error("No custom gene set has enough genes matched to the ranked RNA-seq data. Reduce the minimum size or check the gene IDs.")
            else:
                ...
                try:
                    ...
                    st.session_state["custom_gsea_last"] = {
                        "comparison": comparison,
                        "mode": "custom_gene_sets",
                        "label": label,
                        "results": results,
                        "gene_sets": sets_to_run,
                    }
